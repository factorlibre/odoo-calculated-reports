...
def partners_ledger__get_new_ws(self, _p, _xs, data, objects, wb,
                                sheet_number=1):
    ws = wb.add_sheet(u"{}{}".format(_p.report_name[:29], sheet_number))
...
def partners_ledger_generate_xls_report(self, _p, _xs, data, objects, wb):
    sheet_number = 1
    ws, row_pos = self._get_new_ws(_p, _xs, data, objects, wb,
    ...
    for account in objects:
        if _p['_has_ledger_lines'](account) or \
            ...
            for partner_name, p_id, p_ref, p_name in \
                ...
                for line in _p['_get_ledger_lines_data'](account).get(
                        p_id, []):
                    ...
                    if row_pos >= 65500:
                        sheet_number += 1
                        ws, row_pos = self._get_new_ws(
                            _p, _xs, data, objects, wb,
                            sheet_number=sheet_number)
                # end for line
