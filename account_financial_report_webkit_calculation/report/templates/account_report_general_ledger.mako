...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
        ...
        <%
          ...
          display_ledger_lines = _has_ledger_lines(account)
        %>
            ...
            <div class="act_as_table list_table" style="margin-top: 10px;">
                ...
                <div class="act_as_tbody" style="page-break-inside: avoid">
                      %if display_initial_balance:
                        <%
                        cumul_debit = init_balance.get(account.id, {}).get('debit') or 0.0
                        cumul_credit = init_balance.get(account.id, {}).get('credit') or 0.0
                        cumul_balance = init_balance.get(account.id, {}).get('init_balance') or 0.0
                        cumul_balance_curr = init_balance.get(account.id, {}).get('init_balance_currency') or 0.0
                        %>
                        <div class="act_as_row initial_balance">
                          ...
                          <div class="act_as_cell"></div>
                          ## debit
                          <div class="act_as_cell amount">${formatLang(init_balance.get(account.id, {}).get('debit')) | amount}</div>
                          ## credit
                          <div class="act_as_cell amount">${formatLang(init_balance.get(account.id, {}).get('credit')) | amount}</div>
                          ## balance cumulated
                      ...
                      %endif
                      %for line in _get_ledger_lines_data(account):
                        <%
